...
results = []
for momFactor in [-1, 0, 1]:
    for midDistance in [0.5]:
        for walkUp in [0.15]:
            for walkDown in [0]:
                for minTrade in [0.75]:
                    logFileNameBT  = 'Logs/' + str(momFactor)
...
print('momFactor, midDistance, walkUp, walkDown, minTrade, Return, Sharpe')
print(results[results[:,6].argsort()][::-1])
